...
## Transcript Notes Footer

Every transcript ends with a `## Transcript notes` section. It has two subsections.

### `### Vocabulary corrections applied`

Generated by the build script. Shows each unique correction pattern once, with the total hit count if it fired more than once. Example:

```markdown
### Vocabulary corrections applied

*2 unique pattern(s), 6 total substitution(s).*

- `Ethix` → `ETHIX` (×5)
- `ethichub` → `EthicHub`
```

**Do not** repeat the same correction multiple times (once per occurrence). The count suffix handles repetition. If no corrections were applied, write `*(none)*`.

### `### Notes`

Written by the agent (or a human reviewer) after reviewing the output. This is the high-value section — use it to flag things a reader of the transcript should know:

- **Systematic model errors** — mishearings the vocabulary overlay corrects automatically, and any it *doesn't* catch (e.g., "WhisperX consistently renders 'EthicHub' as 'EthixHub' in Spanish audio — all corrected via overlay")
- **Unresolved proper nouns** — names, titles, organisations that appear garbled and couldn't be fixed (e.g., "Speaker name at [00:35:33] unclear — rendered as 'Rob Verever', likely a different name")
- **Unclear references** — book titles, organisations, URLs that are phonetically ambiguous (e.g., "Book title at [00:37:39] unclear in audio — WhisperX renders it as 'Ullmar Desperanza'; may be a different title")
- **Audio quality issues** — sections with low confidence, background noise, or heavily accented speech where accuracy may be lower
- **Language/translation notes** — for translated transcripts, note systematic choices (e.g., "Technical terms preserved in original language")

If nothing noteworthy was found, write: `*(No reviewer notes.)*`

**Example of a good Notes section:**

```markdown
### Notes

- *"EthixHub"* — WhisperX frequently mishears "EthicHub" as "EthixHub" or "Ethix Hub" in
  Spanish audio; all instances corrected via vocabulary overlay.
- *Speaker name [00:35:33]* — unclear in audio; transcribed phonetically as "Rob Verever".
  Context suggests a named author; could not be verified.
- *Book recommendation [00:37:39]* — title garbled; rendered as "Ullmar Desperanza de
  Andrea Sansa Rollo". Likely a different title — left as-is rather than guessing.
```
